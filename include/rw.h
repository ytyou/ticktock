/*
   Copyright (c) AlexeyAB (git@github.com:AlexeyAB/object_threadsafe.git).
...
namespace tt
{
    ...
    // Author: AlexeyAB (git@github.com:AlexeyAB/object_threadsafe.git)
...
{
public:
    ReadLock(default_contention_free_shared_mutex& mutex) :
        m_mutex(mutex)
    {
        m_mutex.lock_shared();
    }
...
private:
    default_contention_free_shared_mutex& m_mutex;
};

class WriteLock
{
public:
    WriteLock(default_contention_free_shared_mutex& mutex) :
        m_mutex(mutex)
    {
        m_mutex.lock();
    }
