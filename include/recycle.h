...
enum RecyclableType : unsigned char
{
    ...
    RT_DATA_POINT_CONTAINER = RT_DATA_POINT + 1,        // 14
    RT_DOWNSAMPLER_AVG   = RT_DATA_POINT_CONTAINER + 1, // 15
    RT_DOWNSAMPLER_COUNT = RT_DOWNSAMPLER_AVG + 1,      // 16
    RT_DOWNSAMPLER_DEV   = RT_DOWNSAMPLER_COUNT + 1,    // 17
    RT_DOWNSAMPLER_FIRST = RT_DOWNSAMPLER_DEV + 1,      // 18
    RT_DOWNSAMPLER_LAST  = RT_DOWNSAMPLER_FIRST + 1,    // 19
    RT_DOWNSAMPLER_MAX   = RT_DOWNSAMPLER_LAST + 1,     // 20
    RT_DOWNSAMPLER_MIN   = RT_DOWNSAMPLER_MAX + 1,      // 21
    RT_DOWNSAMPLER_PT    = RT_DOWNSAMPLER_MIN + 1,      // 22
    RT_DOWNSAMPLER_SUM   = RT_DOWNSAMPLER_PT + 1,       // 23
    ...
    RT_TAG_MATCHER       = RT_RATE_CALCULATOR + 1,      // 31
    RT_TCP_CONNECTION    = RT_TAG_MATCHER + 1,          // 32
    RT_COUNT             = RT_TCP_CONNECTION + 1        // 33
};
