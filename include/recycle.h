...
enum RecyclableType : unsigned char
{
    ...
    RT_DATA_POINT        = RT_COMPRESSOR_V3 + 1,        // 15
    RT_DATA_POINT_CONTAINER = RT_DATA_POINT + 1,        // 16
    RT_DOWNSAMPLER_AVG   = RT_DATA_POINT_CONTAINER + 1, // 17
    RT_DOWNSAMPLER_COUNT = RT_DOWNSAMPLER_AVG + 1,      // 18
    RT_DOWNSAMPLER_DEV   = RT_DOWNSAMPLER_COUNT + 1,    // 19
    RT_DOWNSAMPLER_FIRST = RT_DOWNSAMPLER_DEV + 1,      // 20
    RT_DOWNSAMPLER_LAST  = RT_DOWNSAMPLER_FIRST + 1,    // 21
    RT_DOWNSAMPLER_MAX   = RT_DOWNSAMPLER_LAST + 1,     // 22
    RT_DOWNSAMPLER_MIN   = RT_DOWNSAMPLER_MAX + 1,      // 23
    RT_DOWNSAMPLER_PT    = RT_DOWNSAMPLER_MIN + 1,      // 24
    RT_DOWNSAMPLER_SUM   = RT_DOWNSAMPLER_PT + 1,       // 25
    RT_HTTP_CONNECTION   = RT_DOWNSAMPLER_SUM + 1,      // 26
    RT_JSON_VALUE        = RT_HTTP_CONNECTION + 1,      // 27
    RT_KEY_VALUE_PAIR    = RT_JSON_VALUE + 1,           // 28
    RT_QUERY_RESULTS     = RT_KEY_VALUE_PAIR + 1,       // 29
    RT_QUERY_TASK        = RT_QUERY_RESULTS + 1,        // 30
    RT_RATE_CALCULATOR   = RT_QUERY_TASK + 1,           // 31
    RT_TAG_MATCHER       = RT_RATE_CALCULATOR + 1,      // 32
    RT_TCP_CONNECTION    = RT_TAG_MATCHER + 1,          // 33
    RT_COUNT             = RT_TCP_CONNECTION + 1        // 34
};
