...
enum RecyclableType : unsigned char
{
    ...
    RT_DATA_POINT        = RT_COMPRESSOR_V2 + 1,        // 12
    ...
    RT_PAGE_INFO_IN_MEM  = RT_PAGE_INFO + 1,            // 27
    RT_QUERY_RESULTS     = RT_PAGE_INFO_IN_MEM + 1,     // 28
    RT_QUERY_TASK        = RT_QUERY_RESULTS + 1,        // 29
    RT_RATE_CALCULATOR   = RT_QUERY_TASK + 1,           // 30
    RT_TCP_CONNECTION    = RT_RATE_CALCULATOR + 1,      // 31
    RT_TIME_SERIES       = RT_TCP_CONNECTION + 1,       // 32
    RT_COUNT             = RT_TIME_SERIES + 1           // 33
};
