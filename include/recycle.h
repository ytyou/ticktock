...
enum RecyclableType : unsigned char
{
    ...
    RT_COMPRESSOR_V3     = RT_COMPRESSOR_V2 + 1,        // 11
    RT_DATA_POINT        = RT_COMPRESSOR_V3 + 1,        // 12
    RT_DOWNSAMPLER_AVG   = RT_DATA_POINT + 1,           // 13
    RT_DOWNSAMPLER_COUNT = RT_DOWNSAMPLER_AVG + 1,      // 14
    RT_DOWNSAMPLER_DEV   = RT_DOWNSAMPLER_COUNT + 1,    // 15
    RT_DOWNSAMPLER_FIRST = RT_DOWNSAMPLER_DEV + 1,      // 16
    RT_DOWNSAMPLER_LAST  = RT_DOWNSAMPLER_FIRST + 1,    // 17
    RT_DOWNSAMPLER_MAX   = RT_DOWNSAMPLER_LAST + 1,     // 18
    RT_DOWNSAMPLER_MIN   = RT_DOWNSAMPLER_MAX + 1,      // 19
    RT_DOWNSAMPLER_PT    = RT_DOWNSAMPLER_MIN + 1,      // 20
    RT_DOWNSAMPLER_SUM   = RT_DOWNSAMPLER_PT + 1,       // 21
    RT_HTTP_CONNECTION   = RT_DOWNSAMPLER_SUM + 1,      // 22
    RT_JSON_VALUE        = RT_HTTP_CONNECTION + 1,      // 23
    RT_KEY_VALUE_PAIR    = RT_JSON_VALUE + 1,           // 24
    RT_MAPPING           = RT_KEY_VALUE_PAIR + 1,       // 25
    RT_PAGE_INFO         = RT_MAPPING + 1,              // 26
    RT_QUERY_RESULTS     = RT_PAGE_INFO + 1,            // 27
