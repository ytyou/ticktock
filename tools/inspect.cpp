...
{
    printf("TSDB: (major=%d, minor=%d, page_size=%d, page_cnt=%d, head_idx=%d, page_idx=%d, start=%" PRIu64 ", end=%" PRIu64 ", actual_cnt=%d, flags=0x%x)\n",
        tsdb_header->m_major_version, tsdb_header->m_minor_version,
        tsdb_header->m_page_size, tsdb_header->m_page_count, tsdb_header->m_header_index,
